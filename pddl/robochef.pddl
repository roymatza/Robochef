...
(:predicates ;todo: define predicates here
    ...
    (visible ?x - env_object ?r - robot) ;true iff the object visible by the robot
    (interactable ?x - env_object ?r - robot) ;true iff the object can be interacted by the robot
...
(:action teleport-robot
    ...
    :precondition (and (visible ?src SELF) (not (visible ?dest SELF)))
    :effect (and (visible ?dest SELF) (not (visible ?src SELF)))
)

(:action make-interactable
    :parameters (?x - env_object)
    :precondition (and (visible ?x SELF) (not(interactable ?x SELF)) )
